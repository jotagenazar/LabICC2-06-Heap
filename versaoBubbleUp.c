...
void heapSort(int *array, int size) {
    //maxHeap já montado pelo bubble up
...
void bubbleUp(int *array, int last) {
    int pai = (last - 1) / 2;

    //se o filho for maior que o pai, ocorre troca e o teste é aplicado para o avô
    if(array[last] > array[pai]) {
        troca(&array[last], &array[pai]);
        bubbleUp(array, pai);
    }
}
